...
use crate::types::{User, Resource, AuthCallbackResponse, UserResponse, ResourceResponse};
use reqwest::Client;
...
impl Service for ServiceImpl {
    async fn verify(&self, code: &str) -> Result<String> {
        ...
        if !auth_response.success {
            return Err(GoIamError::AuthError {
                message: auth_response.message.unwrap_or_else(|| "Authentication failed".to_string()),
            });
        }
    ...
    async fn me(&self, token: &str) -> Result<User> {
        ...
        if !user_response.success {
            return Err(GoIamError::AuthError {
                message: user_response.message.unwrap_or_else(|| "User fetch failed".to_string()),
            });
        }
    ...
    async fn create_resource(&self, resource: &Resource, token: &str) -> Result<()> {
        ...
        if !resource_response.success {
            return Err(GoIamError::AuthError {
                message: resource_response.message.unwrap_or_else(|| "Resource creation failed".to_string()),
            });
        }
...
mod tests {
    ...
    async fn test_verify_malformed_response() {
        ...
        match result.unwrap_err() {
            ...
            _ => panic!("Expected JsonError for malformed JSON"),
        }
    }
